...
ANONData$excluded <- F
ANONData$excluded[c(91,143,47,62,144)] <- T
# what happens if we don't remove them but merely mark them for visualisation?


# what do they actually remove?
val <- ANONData
hist(val$normalized_eigenvalue)
val[c(91,143,47,62,144), ] 

#####

plot = ANONData
plot_data= ggboxplot(plot, x = "diagnosis", y = "normalized_eigenvalue", add = "dotplot",
                     fill = "excluded", # see difference between exclusion and no exclusion
...
ggsave("results/excluded.png", width = 6, height = 5)
# dev.off()
